<template>
  <div>
      <div style="display:flex;justify-content:space-between">
        <el-button type="primary" icon="el-icon-plus" @click="showAddSalaryView">添加工资帐套</el-button>
      ...
      <div style="margin-top:10px">
        <el-table :data="salaries" border stripe>
          ...
          <el-table-column prop="columnProp" label="帐套名称" width="120"></el-table-column>
...
export default {
    ...
    methods:{
      ...
      deleteSalary(data){
        this.$confirm('此操作将永久删除['+ data.name +'工资帐套, 是否继续？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消'
          }).then(() => {
            this.deleteRequest('/salary/sob/delete/' + data.id).then(res => {
              this.initSalaries();
            })
          }).catch(() => {
      ...
      nextStep(){
        if(this.activeItemIndex==10){
          if (this.salary.id){
            this.postRequest('/salary/sob/update', this.salary).then(res=>{
              if(res){
                this.initSalaries();
                this.dialogVisible = false;
              }
            ...
            this.postRequest('/salary/sob/add',this.salary).then(res=>{
              if(res){
                this.initSalaries();
                this.dialogVisible = false;
              }
      ...
      initSalaries(){
        this.getRequest('/salary/sob/get-All').then(res=>{
          if(res){
            this.salaries = res;
          }
        })
