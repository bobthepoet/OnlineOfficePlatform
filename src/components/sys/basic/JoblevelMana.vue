<template>
  <div>
    ...
    <div style="margin-top:10px">
      <el-table :data="jls" stripe border style="width: 70%" size="small" @selection-change="handleSelectionChange">
      ...
      <el-button size="small" style="margin-top:10px;display:flex;justify-content:flex-start" type="danger" :disabled="multipleSelection.length==0" @click="BatchDeleteJobLv">批量删除</el-button>
    </div>
  ...
  methods:{
    BatchDeleteJobLv(){
      this.$confirm('此操作将永久删除[' + this.multipleSelection.length + ']条职称，是否继续', '提示', {
          ...
          this.deleteRequest('/system/basic/joblevel/delete/ids' +ids).then(res => {
            if (res) {
              this.initJls()
            }
          })
    ...
    deleteHandler(index,data){
      this.$confirm('此操作将永久删除[' + data.name + ']职称，是否继续', '提示', {
          ...
          this.deleteRequest('/system/basic/joblevel/delete', {id:data.id}).then(res => {
            if (res) {
                this.initJls()
            }
          })
